...
namespace Agwinterm.Win32ControlTest
{
    ...
    public static class ClipboardGuard
    {
        ...
        /// <summary>Receipt for ONE expected app copy, never an arbitrary current generation. The
        /// caller brackets a posted copy action with the prior receipt and supplies an owner check
        /// against its still-pinned process/window. The check runs while this open excludes another
        /// writer. The expected text must come from the fixture/selection, not the clipboard itself.
        /// Unlike a unique sentinel, an ordinary selection can be copied again by somebody else:
        /// automatic teardown must therefore use RestoreExact, not Restore's content fallback.</summary>
        public static ClipboardWrite ConfirmCopy(string expectedText, uint before, Func<bool> ownerIsOurs)
        {
            if (expectedText == null || ownerIsOurs == null) throw new ArgumentNullException();
            if (!TryOpen()) return new ClipboardWrite { State = "unopened", Detail = "copy receipt could not open the clipboard" };
            try
            {
                uint now = Api.Sequence();
                if (now == before) return new ClipboardWrite { State = "unchanged", Sequence = now };
                if (!ownerIsOurs()) return new ClipboardWrite { State = "changed", Sequence = now, Detail = "copy owner is not the pinned fixture window; nothing touched" };
                ClipboardSnapshot held = TakeOpen();
                string who = Ownership(held, expectedText);
                if (who == "ours") return new ClipboardWrite { State = "written", Sequence = Api.Sequence(), Sentinel = expectedText };
                // The owner IS our app but its data is unreadable/unexpected. Do not call that a
                // foreign copy and discard the baseline: the test may have caused the bad write.
                return new ClipboardWrite { State = "unverified", Sequence = now,
                    Detail = "expected app copy could not be proven; nothing touched" };
            }
            finally { Api.Close(); }
        }

        /// <summary>Automatic multi-copy teardown: restore only at the last PROVEN generation.
        /// A newer generation is preserved even if it holds the very same ordinary selection.
        /// Unknown or failed writes are not receipts; keep the recovery file for explicit recovery.</summary>
        public static ClipboardWrite RestoreExact(ClipboardSnapshot snap, ClipboardWrite receipt)
        {
            if (snap == null || snap.Unsupported != null)
                return new ClipboardWrite { State = "unread", Detail = "no complete original snapshot; nothing touched" };
            if (receipt == null || receipt.State != "written")
                return new ClipboardWrite { State = "unread", Detail = "no proven last-copy receipt; nothing touched" };
            if (!TryOpen()) return new ClipboardWrite { State = "unopened", Detail = "the clipboard could not be opened; nothing restored" };
            try
            {
                uint now = Api.Sequence();
                if (now != receipt.Sequence)
                    return new ClipboardWrite { State = "changed", Sequence = now, Detail = "generation changed after the last proven copy; newer clipboard preserved" };
                string why = PutProven(snap);
                return new ClipboardWrite { State = why == null ? "restored" : "mutated", Sequence = Api.Sequence(), Detail = why };
            }
            finally { Api.Close(); }
        }
