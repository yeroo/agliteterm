...
int main() {
    ...
    for (const std::string mode : { "success", "lost-prepare", "bad-prepare", "lost-create", "garbled-create",
                                   "wrong-id", "wrong-ticket", "refused", "conflict", "pending-conflict", "legacy" }) {
        ...
        auto result = start(mode != "legacy", req, rep, [&](const auto& command, auto& reply, Outcome& outcome) {
            ...
            if (command.which_cmd == agwinterm_ptyhost_Request_prepare_create_tag) {
                ...
                check(std::strcmp(command.cmd.create.creation_ticket, mode == "legacy" ? "" : ticket) == 0);
                if (mode == "lost-create") return false;
                if (mode == "garbled-create") { outcome = Outcome::Undecodable; return false; }
                if (mode == "refused" || mode == "conflict" || mode == "pending-conflict") {
        ...
        check(prepares == (mode == "legacy" ? 0 : 1)); check(creates == (noCreate ? 0 : 1));
        check(cancels == (noCreate || success ? 0 : 1));
        check(result.status == (success ? Status::Created : mode == "conflict" ? Status::Conflict : Status::Failed));
    }
