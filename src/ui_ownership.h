...
inline bool dispatchToUi(const std::string& cmd, const std::string& action = "") {
    if (cmd == "session.overlay" && (action == "text" || action == "copy" || action == "result")) return false;
    if (cmd == "session.type" || cmd == "session.paste" || cmd == "session.write" ||
        cmd == "session.text" || cmd == "session.output" || cmd == "session.copy" || cmd == "session.status") return false;
