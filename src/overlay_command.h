...
inline std::string line(const std::string& encoded) {
    return "[Console]::Write([string][char]27+']133;A'+[char]7+[char]27+']133;C'+[char]7);"
           "$LASTEXITCODE=$null;$c=$null;try{$null=[scriptblock]::Create([Text.Encoding]::Unicode.GetString([Convert]::FromBase64String('"
           + encoded +
           "')));& ([scriptblock]::Create([Text.Encoding]::Unicode.GetString([Convert]::FromBase64String('" + encoded +
           "'))+[char]10+[char]10+'$q=$?;$c=if($null -ne $LASTEXITCODE){$LASTEXITCODE}elseif($q){0}else{1};Set-Variable -Name c -Scope 1 -Value $c'));$q=$?;if($null -eq $c){$c=if($null -ne $LASTEXITCODE){$LASTEXITCODE}elseif($q){0}else{1}}}catch{$c=1;Microsoft.PowerShell.Utility\\Write-Error -ErrorRecord $_ -ErrorAction Continue};"
