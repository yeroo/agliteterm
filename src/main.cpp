    ...
    if (g_th.dark && !g_th.classic) {
        ...
        if (m == WM_PAINT) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                // long name overflowed (lite #22). The ellipsis handles what does not fit the part.
...
// True when a workspace was erased. False = nothing changed: the index is past the list (a
// concurrent delete already took it — the verb resolved its index outside the lock, #21) or it is
// the last workspace; the caller must not say "deleted" then (lite #22).
static bool deleteWorkspace(int w) {
    {   // under g_lock: reached from workspace.delete on a pipe thread while `tree` (another pipe
        ...
        LockG hold;
        if ((int)g_workspaces.size() <= 1 || w < 0 || w >= (int)g_workspaces.size()) return false;
...
static std::string ctlDispatch(const std::string& line) {
    ...
    if (cmd == "workspace.delete") {
        ...
        if (!deleteWorkspace(w)) return ctlErr("nothing deleted: the workspace is already gone, or it is the last one");
