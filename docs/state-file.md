...
return. Ordinary line types replace each with a space (`tsvField`). P9's `R`/`B` and P10b's `K2` command fields
...
| `E` | `i`… | The indices, among the `S` lines, of sessions whose **empty** argument list is exact: a `session new --command-mode direct` launch of a program with no arguments (#79). Without it an empty argument list means "no arguments given", and a PowerShell app relaunches with its usual `-NoLogo -NoExit -EncodedCommand` wrapper. A non-empty argument list is always relaunched exactly, so it is never listed. Absent when none; older builds skip it. |
...
| `K` | `i` `pane0` `pane1` | The **captured commands** of the session at `S` index `i` (parity batch P3, `restore capture`): `pane0` is the slot of the session's own shell, `pane1` the slot of its split shell (empty when there is no split, or nothing was captured there). An empty field is "none"; a slot is a plain string with no rules of its own, so unlike `C` one line carries both panes. One line per session with at least one slot. **The fields are by ROLE, not by position on screen**: `pane0` is always the session's own shell and `pane1` always the split shell, whatever slot each sits in — the `L` line carries the order, `K` does not repeat it. After a promotion (the session's own shell closed, the split shell became the session) the survivor's slot is `pane0`, because it is the session's own shell now. Written after the `P` lines by convention — `K` sits with the `P` lines it describes — not by requirement: the reader collects every line type in file order and applies them in its own fixed order, so a `K` above a `P` restores identically. A slot is a checkpoint a caller reads back (`tree --json`, `capturedCommands`); P10b may replay it only when `restore-commands` is enabled (default false). The writers are `restore capture` and, when `restore-commands` is enabled, **the close itself**: OnDestroy captures every real pane while its shell is still alive, before the final save, so the checkpoint describes the moment the window went away (agwinterm's `SaveState(captureCommands: true)`). Malformed owner indices are refused, never coerced to zero. |
| `K2` | `i` `pane0` `pane1` | P10b lossless captured slots, with the same roles and owner guards as K. Written instead of K when either capture contains a tab, CR or LF; both fields then use the R/B JSON-content codec. Older builds ignore K2 and drop those captures on their next save; ordinary captures retain legacy K. |
...
current binding, otherwise its current pin, otherwise its captured K/K2 slot only when
`restore-commands` is enabled, then writes outside the state lock. It is a delay, not a readiness
guarantee. Adopted/exited/gone/read-only panes are skipped. Changes before the timer fires are
honored. Captured replay defaults off; explicit pins and bindings do not depend on that setting.
