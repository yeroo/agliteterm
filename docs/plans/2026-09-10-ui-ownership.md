...
- Agent status and overlay result/copy/text reads also remain worker-owned. Modal editing does
  not suppress liveness updates. An input request whose pane vanished before the I/O lookup refuses
  instead of falling back to an unreserved raw write; a gate already found survives later unlisting.
...
- A surviving session menu follows its current workspace; Rename finds its rebuilt tree row.
  Concurrent acceptance checks preserve baseline workspace names/IDs and session placement too.
